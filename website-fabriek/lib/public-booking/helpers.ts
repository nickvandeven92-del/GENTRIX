...
/** Maandraster (maandag eerste kolom); cellen buiten huidige maand hebben inMonth false. */
...
/**
 * Publieke boek-flow: zelfde endpoints als `/api/public/clients/[slug]/…`.
 * Portaal beheert diensten, medewerkers, planning en booking_settings — geen aparte Bookflow-backend.
 */
export const PUBLIC_BOOKING_API = {
  slots: (encSlug: string) => `/api/public/clients/${encSlug}/booking-slots`,
  staff: (encSlug: string) => `/api/public/clients/${encSlug}/booking-staff`,
  services: (encSlug: string) => `/api/public/clients/${encSlug}/booking-services`,
  appointments: (encSlug: string) => `/api/public/clients/${encSlug}/appointments`,
} as const;
