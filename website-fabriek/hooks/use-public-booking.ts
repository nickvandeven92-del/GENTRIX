...
type UsePublicBookingArgs = { slug: string };

export function usePublicBooking({ slug }: UsePublicBookingArgs) {
  const enc = encodeURIComponent(slug);
  const slotsApi = PUBLIC_BOOKING_API.slots(enc);
  const staffApi = PUBLIC_BOOKING_API.staff(enc);
  const servicesApi = PUBLIC_BOOKING_API.services(enc);
  const bookApi = PUBLIC_BOOKING_API.appointments(enc);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const json = (await res.json()) as { ok?: boolean; meta?: BookingMeta; error?: string };
