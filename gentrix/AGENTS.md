...
Chameleon needs `.env` in `frontends/chameleon/` (see `.env.example`). **Boeken:** `npm run build` bouwt de Vite-SPA naar `public/booking-app/`; studio-CTA’s → `/booking-app/book/{slug}`; `/boek/{slug}` redirect daar naartoe (bookmarks). Lokaal eenmalig `npm run build:booking-app` vóór `/boek/…` testen, of `npm run dev:with-booking-spa`. Zie `docs/BOOKING_GENTRIX_UNIFIED.md`.
