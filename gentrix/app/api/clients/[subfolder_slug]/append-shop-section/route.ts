...
import { appendDefaultShopSectionToSections } from "@/lib/site/append-booking-section-to-payload";
...
/**
 * Voegt de canonieke webshop-sectie (vier producttegels) toe aan de huidige draft-site (geen AI).
 */
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const merged = appendDefaultShopSectionToSections(siteParsed.sections, {
      ...
      return NextResponse.json(
        {
          ...
          code: "shop_exists",
          error:
            'Er staat al een sectie met id "shop". Stuur replace_existing: true om die te vervangen door de standaard-blok.',
        },
        { status: 409 },
      );
    }
    ...
    const persist = await persistTailwindDraftForExistingClient(
      ...
      twPayload,
      {
        ...
        snapshotLabel: merged.replaced ? "Webshop-sectie vervangen (standaard)" : "Webshop-sectie toegevoegd (standaard)",
        snapshotNotes: "Zonder AI — vast sjabloon met vier producttegels + webshop-pad",
