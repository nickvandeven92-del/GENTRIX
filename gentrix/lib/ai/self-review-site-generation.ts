...
- **Multipage nav-pariteit:** als \`marketingPages\` of \`contactSections\` aanwezig is: vergelijk de primaire \`<header>\` **class**-string (en zo mogelijk de inner \`<nav>\`) tussen landing en elke subroute; wijk de chrome **structureel** af (andere achtergrond, border, typografie-gewicht op de balk) → trek gelijk aan de **landings**-header (zelfde utility-reeks), behoud per route alleen actieve-link state (\`aria-current\`, underline).
...
- **Tweede pseudo-hero / dubbele split:** als onder de hero nog een full-viewport **split** (tekst | groot media) staat met **dezelfde** dramatiek en vergelijkbare hoofdbelofte als de hero, herschik naar band/grid/editorial **zonder** sectie-\`id\` of volgorde te wijzigen — behoud impact, vermijd “twee keer hetzelfde theater”.
- **Lege mediakolom:** grote kale/zwarte placeholder naast copy → vul met \`background-image\`/gradient met textuur, typografie-led, of **klantfoto-URL** — **geen** externe stock-URL's; of verwijder het split-patroon ten gunste van enkelkolom — **geen** lege decoratieve rechterkolom.
- **Dubbele merken-/“wij werken met”-laag:** als \`stats\`/\`brands\` (of vergelijkbaar trust-blok) al logo’s/cijfers toont, **geen** tweede identieke strook in \`features\`/\`footer\`; snoei of fuseer tot **één** rustige trust-presentatie (zonder echte partners uit de briefing te verzinnen).
- **Screenshot als hele reviews-sectie:** als een testimonial-/“wat klanten zeggen”-blok vooral één enorme \`<img>\` is (Google-/desktop-scherm), **vervang** door kaarten met korte NL-quotes (inhoud uit briefing/claims) + nette borders; verwijder de screenshot-\`<img>\` tenzij de briefing expliciet vraagt om die afbeelding 1:1.
...
- **Hero te eenduidig (sjabloon):** als \`#hero\` alleen kop + één regel op kale gradient is **zonder** bijkomende visuele laag (beeld, scrim+bento, trust-rij, SVG-ornament, asymmetrisch paneel) terwijl briefing/branche dat toe laat: **verrijk** binnen dezelfde sectie-\`id\` (geen nieuwe secties) volgens pass-1 **hero-variatie** — geen tweede alinea of infodump.
- **Hero = geen praktische blokken:** als \`#hero\` openingstijden (per dag), volledig adres, kaart-embed of een uitgebreid contactpaneel bevat → **verplaats** naar \`#footer\` of \`contactSections\` (behoud sectie-\`id\`’s en volgorde); laat in de hero alleen kop + korte regel + CTA’s + hoogstens mini trust (ster/cijfer), **geen** urenlijst.
- **Telefoon / WhatsApp / openingstijden — dedupe:** als dezelfde \`tel:\`, \`wa.me\` of dezelfde volledige openingstijdenzin **meer dan tweemaal** op de landing voorkomt (hero-strip + split + CTA + footer), **snoei** tot **max. twee** plekken (bijv. één compacte CTA + footer) — identieke infoblokken in opeenvolgende secties samenvoegen of één verwijderen; **geen** secties of \`id\`'s wijzigen.
- Behoud \`data-animation\`, \`data-aos\` (AOS) en root-\`id\` op sectie waar geldig. **Marquee/ticker:** **verboden** — verwijder \`studio-marquee\`, \`studio-marquee-track\` en \`<marquee>\` (logo's/trust **stilstaand**). **\`studio-border-reveal\`:** behoud bestaande \`studio-border-reveal--h\` / \`--v\` waar ze kloppen; als de briefing **rand/border/kader/scroll-lijn** vraagt en er staan **minder dan 3** reveals op de landing → voeg lege \`<div class="… studio-border-reveal studio-border-reveal--h …">\` toe onder koppen of tussen blokken (met passende \`[--studio-br-rgb:…]\` als het palet dat vraagt). **Laser:** als \`studio-laser-*\` staat zonder duidelijke cyber/neon/sci-fi-wens → **verwijder**; liever border-reveal + \`data-animation\`. Als wél passend: max. één rail in hero; \`relative\` parent.
- Als de briefing **motion-signalen** bevat (interactief, dynamisch, animatie(s), motion, micro-interactions/micro-interacties, scroll-animaties, **animated on scroll** / scroll-driven / scroll-triggered, **AOS**, beweegbare secties, in beweging, bewegende UI, levend/levendige site, niet te statisch, veel visuele dynamiek — NL of EN) en het concept **weinig of geen** \`data-animation\` / \`data-aos\` heeft: voeg **minstens 10** \`data-animation="fade-up"\` en/of \`data-aos="fade-up"\` (of andere AOS-waarden) toe op zichtbare elementen — **zonder** sectie-\`id\` of volgorde te wijzigen. **Niet** beide attributen op **dezelfde** node.
...
- **Hero heel kaal (effen donker zonder enige visuele laag):** alleen bijsturen als dat duidelijk zwak oogt **en** de briefing geen expliciet minimalisme vraagt — voeg eventueel foto, gradient of textuur toe.
- **Hero-kop overlappende letters:** als een \`h1\`/display **verticaal op elkaar valt** (vaak door \`leading-none\`, te agressieve \`-space-y-*\` of vaste hoogte op de verkeerde node): verhoog \`leading-\*\` (\`leading-tight\` → \`leading-snug\` of \`leading-[1.1]\`) en/of breek de kop over 2 regels met gezonde regelafstand; behoud sectie-\`id\`.
...
- **Verboden desktop-zijmenu:** als je een vaste rechter-/linkerkolom ziet als tweede nav (\`fixed top-0 right-0 h-full\`, \`fixed ... left-0 ... h-full\`) naast de hoofdtopbar, behandel dat als mobiele drawer en verberg op desktop (\`lg:hidden\` of equivalent) óf verwijder de duplicaatkolom.
- **Mobiel menu — Alpine-scope:** als hamburger en × **tegelijk** in één knop staan of de knop reageert niet: verplaats \`x-data\` naar **\`<header>\`** (of één wrapper die knop + sheet + alle \`x-show\` op die state bevat) en laat \`@click\`/\`x-show\` dezelfde sleutel gebruiken (\`navOpen\`, \`open\`, …).
...
`
  ...
  const strictOrderLine =
    pi?.sections?.length && pi.sections.length > 0
      ? pi.sections.map((id) => `${b}${id}${b}`).join(" → ")
      : `${b}hero${b} → ${b}stats${b}/${b}brands${b} → ${b}steps${b}/${b}features${b} → ${b}footer${b}`;
