...
- **Nav-chrome:** primaire \`<header>\` \`class=\` (en inner \`<nav>\`-utilities) zijn op **landing, elke marketing-key en contact** inhoudelijk **gelijk**; alleen actieve link verschilt.
...
- **Zelfde site-chrome (nav) binnen ??n multipage-site:** geldt **alleen** voor **deze** JSON (landing + subpagina???s + contact): kies **??n** header-uitstraling voor de **hele** site (licht, donker, glas, pill, effen, ???) afstemmen op \`config.theme\` + briefing ??? **niet** op de landing een andere nav-stijl dan op marketing/contact. **Tussen verschillende generaties/sites** is de nav-vorm **vrij** ??? wissel archetype (geen verplichting om steeds transparant???scroll of dezelfde pill). Kopieer de primaire landings-\`<header>\` (zelfde **volledige** \`class=\` op \`<header>\` **en** dezelfde utility-reeks op de inner \`<nav>\`, logo-wrapper, link-stijlen en contact-knop) naar **elke** \`marketingPages\`-route en naar \`contactSections\`; alleen **actieve route** mag visueel afwijken (bv. dunne underline, \`aria-current="page"\`). Zelfde **linkstructuur**: \`__STUDIO_SITE_BASE__/???\` + \`__STUDIO_CONTACT_PATH__\`; elke marketing-key **behalve** \`faq\` minstens **??n keer** als \`__STUDIO_SITE_BASE__/<slug>\`. **Geen** \`__STUDIO_SITE_BASE__/faq\` in \`<header>\` ??? FAQ alleen in de **footer** op de landing.
...
function buildOperationalNavParts(
  ...
  const section4Nav = preserve
    ...
    : `- **Navigatie:** zie ?3 ??? **exact ??n** globale site-nav (??n \`<header>\`/\`<nav>\`-blok met alle ankers); **geen** tweede verticale/zijbalk met dezelfde linkset naast de header. Eerste header is bij master config een **infer-bron** (simpel houden); \`href="#???"\` matchen sectie-\`id\`'s. **Vorm vrij** op die bron zolang de nav bereikbaar blijft.\n`;
...
- **Nav chrome-pariteit (binnen ??n site):** zie multipage-blok 3B ??? de primaire \`<header>\` / inner \`<nav>\` moeten op **alle** routes van **deze** site dezelfde Tailwind-utilities op de chrome dragen (geen mix van donk/transparant op ??n pagina en strak wit op een andere), behalve actieve-link state. Dit beperkt **niet** hoe andere sites in andere runs eruitzien.\n`
    : "";
...
3. **Mobiel + navigatie:** werkende responsive layout en **precies ??n** globale navigatie boven de vouw (\`<header>\`/\`<nav>\` ??? zie ?3 + Alpine-blok). **Geen dubbele navbar**. **Standaard (master + infer):** eerste \`<header>\` is **simpel** (\`sticky top-0\` + \`z-*\`, merk + genoeg \`<a>\` voor infer) ??? **geen** \`fixed\` top-bar, **geen** \`@scroll.window\`/\`studio-nav-scroll-dim\`/\`data-gentrix-scroll-nav\` op die host; de **definitieve** fixed app-shell rendert de studio. **Als infer faalt** blijft jouw header staan ??? volg dan de **legacy**-regels in het Alpine-blok. ${marketingMultiPage ? "Subpagina-links via __STUDIO_SITE_BASE__ (zie ?3B); " : ""}Landings-\`id\`'s consistent met \`href="#???"\` **alleen binnen dezelfde pagina**; link ???Contact??? naar \`__STUDIO_CONTACT_PATH__\` ${marketingMultiPage ? "(verplicht token)" : ""}.
...
**Vrijheid:** ${strictLanding && strictLandingSectionIds ? `Binnen de **${strictLandingSectionIds.length}** vaste landings-secties (zie STRIKTE LANDINGS) is visuele uitwerking vrij ??? **geen** wijziging van volgorde of \`id\`'s.` : "hero, secties en lay-out stem je af op de **briefing**; geen verplicht sjabloon (editorial, kaarten, foto-hero, typografie-led ??? allemaal toegestaan)."} **Nav:** wissel vrij tussen balk vs. pill-**look** op de **bron**-\`<header>\` (infer leest o.a. \`rounded-full\`); **geen** zware custom scroll/overlay-stack daarop ??? zie Alpine-blok. De bibliotheek \`studio-nav-scroll-dim\` is **alleen** voor legacy / headers die **niet** worden vervangen. Multipage: **consistente** nav-inhoud tussen routes van dezelfde site.

**Navigatie (${marketingMultiPage ? `multi-page: landing + marketing-subroutes (${mpKeysLine || "zie ?3B"}) + contact` : "one-pager"}):** **??n** globale nav met **merk/bedrijfsnaam** en bruikbare links (minstens **twee** bruikbare anchors + merk zodat infer kan overnemen). ${marketingMultiPage ? `In de **top-\`<header>\`**: voor elke marketing-key **behalve** \`faq\` een link \`__STUDIO_SITE_BASE__/<slug>\` (slugs: ${mpKeysLine || "zie ?3B"}); **\`faq\` niet** in de header ??? FAQ-link in de **footer** van de landing. **Geen** \`#???\`-nav naar inhoud die op een subroute staat. **Multipage:** de **zelfde** linkset/merk op landing, elke marketing-route en contact ??? zie 3B; alleen actieve route mag afwijken (underline / \`aria-current\`). ` : ""}Op **??n** pagina: \`href="#sectie-id"\` alleen naar id's op **die** pagina. **Contact:** \`href="__STUDIO_CONTACT_PATH__"\`. **Geen tweede** volledige menu. Primaire CTA mag in de nav en/of **??n** vaste floating knop. **Bron-header:** \`sticky top-0\` + \`z-*\` (geen \`fixed\` top-bar in AI-output); **vorm** (pill/blur/effen) vrij op die bron. ${marketingMultiPage ? "Zelfde nav-keuze **identiek** op alle pagina's. " : ""}**Nav-contrast** op de zichtbare (gerenderde) balk regelt de studio ??? toch **nooit** witte tekst op lichte **bron**-nav zonder contrast (voor het zeldzame pad zonder vervanging). **Navbar vs split-hero:** \`<header>\`/\`<nav>\` **niet** in dezelfde twee-koloms-split als de hero ??? **volle-breedte** balk boven de split. **Verboden:** permanente zij-nav-kolom naast de hoofdnav op desktop. **Als jouw header blijft staan (legacy / infer faalt):** mobiel menu **gesloten** bij load; **\`x-data\`** op \`<header>\` (of ??n wrapper rond knop + sheet) zodat hamburger/\`x-show\` dezelfde scope delen. **Niet** leveren zonder zichtbare site-nav boven de vouw.
