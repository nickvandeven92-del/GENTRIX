...
- **Navigatie (studio shell):** bouw **geen** eigen top-\`<header>\`/\`<nav>\`-chrome (geen vaste balk, geen scroll/fixed-nav utilities op een primaire header). Vul **\`config.studioNav\`** met \`brandLabel\`, \`brandHref\`, \`items[]\` (alle routes dezelfde labels/hrefs), optioneel \`cta\`, optioneel \`navVisualPreset\` (één van: minimalLight, darkSolid, glassLight, floatingPill, luxuryGold, editorialTransparent, softBrand, compactBar), optioneel \`navVisualOverrides\` (**alleen** \`height\` / \`ctaStyle\` / \`activeIndicator\`), optioneel \`navBarLayout\`: \`"standard"\` (standaard) of \`"centeredLinks"\` wanneer de briefing **gecentreerde menulinks** wil (merk links, linkcluster midden op desktop, CTA rechts) ??? **zet dit veld** i.p.v. eigen flex-markup in secties. **Optioneel** \`"studioShellNav": true\` in \`config\`: dan **moet** \`studioNav\` geldig zijn (server valideert; geen infer uit AI-\`<header>\`). **FAQ** staat **niet** in \`studioNav.items\` ??? alleen footer-link \`href="__STUDIO_SITE_BASE__/faq"\` op de landing. Elke marketing-key **behalve** \`faq\` wél als item met \`href="__STUDIO_SITE_BASE__/<slug>"\`.
