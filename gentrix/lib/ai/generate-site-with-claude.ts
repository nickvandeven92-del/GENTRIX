...
**Hero (\`#hero\`):** eerste indruk = **typografie + (optioneel) eigen beeld of gradient**, niet een **brochure**: zie **COPY — MINDER IS MEER** — **geen** twee alinea’s onder de kop; **geen** openingstijden, volledig adres of contactkaart in de hero (dat hoort in footer/contact). **Één primaire CTA** en optioneel **één secundaire** (werkende \`href\`). **Verboden:** decoratief **scroll**-label in de hero. Volg **§VIEWPORT**. **Geen** externe stock in de hero — **wel** klant-upload-URL's als die sterk genoeg zijn; anders split met **gradient/textuur/SVG**. **Achtergrond-\`<video>\`:** alleen met **concrete https-URL** in de briefing; anders gradient + \`data-animation\` / AOS. Houd je aan **BRANCHE-INSPIRATIE**.
