...
function buildOperationalNavParts(
  ...
  const section4Nav = preserve
    ...
    : `- **Navigatie (studio shell):** vul \`config.studioNav\` (merk, \`items\`, \`cta\`, optioneel \`navVisualPreset\` / \`navVisualOverrides\` / \`navBarLayout\` \`standard\`|\`centeredLinks\` bij wens “links gecentreerd”; \`navChromeTheme\` voor nav-eigen \`primary\`/\`accent\` hex; **floating** = \`floatingPill\` of het woord \`floating\` in \`navVisualPreset\`). Zet optioneel \`"studioShellNav": true\` om server-side af te dwingen dat \`studioNav\` geldig is (anders faalt compose); lek-chrome in secties wordt gestript + gelogd. **Geen** eigen top-\`<header>\`/\`<nav>\`-chrome in sectie-HTML ??? de server bouwt de vaste navbar. **Geen** tweede verticale menubalk. Sectie-\`href="#???"\` alleen voor ankers op **dezelfde** pagina.\n`;
