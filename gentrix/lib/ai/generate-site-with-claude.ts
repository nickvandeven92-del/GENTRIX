...
3. **Mobiel + navigatie:** werkende responsive layout en **precies ??n** globale navigatie boven de vouw (\`<header>\`/\`<nav>\` ??? zie ?3). **Geen dubbele navbar** met dubbele linksets. Sticky balk, pill of fixed: allemaal ok?; ${marketingMultiPage ? "subpagina-links via __STUDIO_SITE_BASE__ (zie ?3B); " : ""}landings-\`id\`'s consistent met \`href="#???"\` **alleen binnen dezelfde pagina**; link ???Contact??? naar \`__STUDIO_CONTACT_PATH__\` ${marketingMultiPage ? "(verplicht token)" : ""}.
