...
function buildOperationalNavParts(
  ...
  const section4Nav = preserve
    ...
    : `- **Navigatie:** zie ?3 ??? **exact ??n** globale site-nav (??n \`<header>\`/\`<nav>\`-blok met alle ankers); **geen** tweede verticale/zijbalk met dezelfde linkset naast de header. \`href="#???"\` matchen sectie-\`id\`'s. **Vorm vrij** zolang de nav bereikbaar blijft.\n`;
...
3. **Mobiel + navigatie:** werkende responsive layout en **precies ??n** globale navigatie boven de vouw (\`<header>\`/\`<nav>\` ??? zie ?3). **Geen dubbele navbar** met dubbele linksets. **Primaire site-nav:** altijd \`sticky top-0\` met duidelijke \`z-\` (geen \`fixed\` top-bar); pill/blur/Alpine blijven vrij. ${marketingMultiPage ? "Subpagina-links via __STUDIO_SITE_BASE__ (zie ?3B); " : ""}Landings-\`id\`'s consistent met \`href="#???"\` **alleen binnen dezelfde pagina**; link ???Contact??? naar \`__STUDIO_CONTACT_PATH__\` ${marketingMultiPage ? "(verplicht token)" : ""}.
