...
- **Afbeeldingen:** zie **Beeld (studio)** — **spaarzaam**; geen volsite vol **\`<img>\`**-kaarten. **Geen** anonieme externe stock-URL's. **Hero:** gradient/typografie of **klant-URL** uit **KLANTFOTO'S**; optioneel vult de server één AI-hero in.
...
1. \`hero\` — **alleen** krachtige kop + **max. één** ultrakorte regel (≤12 woorden); **max. 2** CTA-links (\`<a>\` met button-styling); **max. 3** social-proof-items (alleen uit briefing). **Geen** tweede alinea of “vertel”-tekst in de hero. **Geen** openingstijden/adres/kaart/contactpaneel in de hero — zie **COPY — MINDER IS MEER**. **Split/immersive:** mediakant **gevuld** met **gradient/textuur/SVG** of **klantfoto-URL** — **geen** externe stock; **geen** lege/zwarte placeholderkolom.
...
- **Nav (one-pager):** **exact één** globale navigatie (\`#hero\` of eerste sectie: één \`<header>\`/\`<nav>\` met merk + **alle** interne links). **Verboden:** dezelfde menu-items **tweemaal** (bv. verticale linkkolom in de hero **én** horizontale topbar) — dat voelt als twee sites in één. Hamburger/overlay telt als **dezelfde** nav, geen tweede kopie. **Vorm vrij** (sticky, pill, fixed, …); nav moet bruikbaar blijven bij scroll. **Nav-contrast (verplicht):** link- en logo-kleur moet **leesbaar** zijn op de gekozen nav-achtergrond — **verboden** o.a. \`text-white\` op \`bg-white\`, \`bg-stone-50\`, \`bg-neutral-100\` of andere lichte vlakken; gebruik dan \`text-stone-900\` / \`text-neutral-900\` (evt. \`border-b\`). Alleen \`text-white\`/\`text-stone-100\` op **donkere** balk (\`bg-stone-900\`, \`bg-black\`, \`bg-black/60\` + backdrop) of op een **duidelijke** frosted strip met voldoende contrast. **Fixed/sticky boven hero:** gebruik je \`fixed top-0\` (of overlay boven beeld), geef \`#hero\` dan **minimaal** \`pt-20 sm:pt-24\` (of vergelijkbare ruimte) op de **inhoud** onder de balk — anders valt de kop onder de nav (Studio corrigeert dit deels met CSS, maar de markup moet kloppen). **Navbar vs split-hero:** zet \`<header>\`/\`<nav>\` **niet** in dezelfde \`grid\` / twee-koloms-split als de hero-inhoud (half licht / half donker) — gebruik een **volle-breedte** balk (\`w-full\`) met **één** duidelijke achtergrond boven de split, zodat de menubalk niet visueel “mee breekt” met de hero. **Mobiel:** \`x-data\` op \`<header>\` (of één parent van knop + sheet + \`x-show\`-iconen) — anders geen werkende toggle.
...
**Hero (\`#hero\`):** eerste indruk = **typografie + (optioneel) eigen beeld of gradient**, niet een **brochure**: zie **COPY — MINDER IS MEER** — **geen** twee alinea’s onder de kop; **geen** openingstijden, volledig adres of contactkaart in de hero (dat hoort in footer/contact). **Één primaire CTA** en optioneel **één secundaire** (werkende \`href\`). **Verboden:** decoratief **scroll**-label in de hero. Volg **§VIEWPORT**. **Standaard geen** strakke **50/50 grid** (foto-kolom | tekst-kolom) — gebruik liever **full-bleed** beeld met overlay-tekst (zie VIEWPORT), tenzij de briefing expliciet split vraagt. **Geen** externe stock in de hero — **wel** klant-upload-URL's als die sterk genoeg zijn; anders **gradient/textuur/SVG** i.p.v. generieke split-layout. **Achtergrond-\`<video>\`:** alleen met **concrete https-URL** in de briefing; anders gradient + \`data-animation\` / AOS. Houd je aan **BRANCHE-INSPIRATIE**.
...
/** Ruimte voor zelfreview + AI-hero + journal vóór Vercel het verzoek beëindigt. */
