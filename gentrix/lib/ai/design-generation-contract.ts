...
export function buildDesignContractPromptInjection(
  ...
  const lines = [
    ...
    "- **Geen** externe stock-foto-URL's in de HTML — gebruik **klant-uploads**, gradient/SVG, of **`<video>`** met echte briefing-URL. **(Studio)** met Google AI Studio (Gemini image) of OpenAI + opslag aan kan de server **ná** generatie één unieke AI-fotografie in `#hero` injecteren; houd `<section id=\"hero\"` dus **`relative overflow-hidden`** zodat een achtergrondlaag netjes onder de inhoud blijft. **Die server-AI-hero mag nooit fictieve personen of gezichten tonen** — alleen omgeving, **macro/materiaal**, architectonisch fragment, rekwisieten/stilleven of abstracte branche-sfeer (vermijd generieke brede kantoor-stock); echte portretten/team alleen via **KLANTFOTO'S** in de HTML.",
    "- **Hero:** vertaal `heroVisualSubject` naar compositie/kleur/motion **zonder** anonieme stock-foto's en **zonder** AI-mensen; geen stock-video zonder URL in de briefing.",
    "",
