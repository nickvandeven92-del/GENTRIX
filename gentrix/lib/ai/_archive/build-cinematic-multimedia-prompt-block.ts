...
function buildHeroRequirementParagraph(mandatoryVideo: boolean): string {
  if (mandatoryVideo) {
   ...
   - \`<video class="absolute inset-0 h-full w-full object-cover" muted playsinline autoplay loop preload="metadata" poster="https://…">\` met een **werkende** MP4-URL op \`https://\` (eigen CDN of briefing-URL), **én** een \`poster\` met **echte** https-URL uit de briefing. **Verboden:** hero zonder video in deze modus; geen tweede autoplay-video elders; geen slap effen vlak zonder bewegend beeld.`;
  }
  return `1. **Hero (eerste sectie \`hero\`):** Dominant **beeld of video** over minimaal **~60–100vh** — \`relative overflow-hidden\` met een **achtergrondlaag**:
   - Voorkeur: \`<video … poster="https://…">\` met een **werkende** MP4-URL op \`https://\` (briefing), **én** een \`poster\` met echte URL voor eerste frame; **of** full-bleed **klantfoto** of **AI-hero** met **lichte tot middel** lees-overlay óf donkere overlay **alleen** als contrast dat nodig heeft — **niet** een effen kleurvlak zonder media.`;
}
