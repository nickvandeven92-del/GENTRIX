...
export function validateGeneratedPageHtml(html: string, plan: HomepagePlan): GeneratedPageValidation {
  ...
  const buttonWarnThreshold = 14;
  if (buttonCount > buttonWarnThreshold) {
    warnings.push(`Veel buttons (${buttonCount}); overweeg max ~${buttonWarnThreshold} voor rust.`);
  }
  ...
  const cardBudgetBase =
    plan.densityProfile === "dense_commerce" || plan.experienceModel === "ecommerce_home"
      ...
      : editorialLean
        ? 7
        : 10;
  const cardBudget = Math.ceil(cardBudgetBase * 1.65);
  if (cardish > cardBudget) {
    warnings.push(
      `Veel ‘card’-verwijzingen (${cardish}); overweeg meer variatie (open vlakken, lijsten, split) als de pagina repetitief oogt (kaartenrij mag als het past bij retail/features).`,
    );
  }
