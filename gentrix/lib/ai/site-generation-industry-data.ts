...
- **Hero-idee:** krachtige **typografie + gradient/textuur** (eventueel **klantfoto**-URL uit de opdracht); kop in **elegant serif**, kort (paar woorden), **geen** lange paragraaf in de hero — uitleg pas onder de vouw. Vermijd "billboard" all-caps extrabold sans + twee grote knoppen (voelt goedkoop). **Geen** achtergrond-\`<video>\` tenzij de briefing **expliciet** om bewegende achtergrond / video / loop vraagt — **geen** anonieme stock in de hero (alleen \`gallery\` of klant-uploads).
...
- **Beeld:** zonder \`gallery\`: geen stock — werk met typografie/gradient. **Met \`gallery\`:** daar pas Unsplash-zoektermen zoals "woman hair salon", "hair coloring", "balayage hair", "bridal hairstyle".
...
- **Hero-idee:** full-bleed **water / ochtendnevel / boot / vislijn** als **gradient + typografie** (of klantfoto); kop in **elegant serif** + korte subkop. **Stock-foto's** alleen in een aparte \`gallery\` met passende zoektermen (fishing rod lake, fly fishing, …). Donker+diep **of** licht+editorial — kies één lijn en volg die.
...
- **Hero-idee (VERPLICHT visuele diepte):** **geen** kale zwarte of grijze vlakken. Voorkeur: **gelaagde gradient + textuur/typografie** (abstract digitaal, bokeh-sfeer, stad bij nacht — als **CSS**, geen anonieme stock-URL in \`#hero\`). **Of** een echte \`<video>\`-achtergrond **alleen** als de briefing een **concrete https-MP4/WebM-URL** bevat (eigen upload); **geen** stock-video zonder URL. **Subtiele variant** (rustig, niet druk): mag — zolang er **gelaagde gradient + textuur/decor** is (geen effen \`bg-black\`). Eventuele **stock-rasters** alleen in \`id: "gallery"\`.
