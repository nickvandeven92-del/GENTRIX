 ...
 */
export const STUDIO_DEFAULT_ONEPAGE_HEADER_ALPINE_SNIPPET = `<header id="site-header" data-studio-skip-nav-tone class="fixed inset-x-0 top-0 z-[80] border-b border-white/10 bg-slate-950/90 shadow-[0_1px_0_0_rgba(255,255,255,0.06)] backdrop-blur-md" x-data="{ navOpen: false, _il(v){try{var io=/iP/.test(navigator.userAgent)||(navigator.maxTouchPoints>1&&/Mac/.test(navigator.platform));if(io)document.documentElement.style.cssText=v?'overflow:hidden;position:fixed;width:100%':''}catch(_){}}, init(){this.$watch('navOpen',v=>this._il(v))} }" @keydown.escape.window="navOpen = false">
  <div class="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-4 md:px-6">
    ...
    <nav class="hidden items-center gap-8 text-sm font-medium lg:flex" aria-label="Hoofdmenu">
      <a href="#features" class="text-white/90 transition-colors hover:text-white">Diensten</a>
      <a href="__STUDIO_SITE_BASE__/over-ons" class="text-white/90 transition-colors hover:text-white">Over ons</a>
